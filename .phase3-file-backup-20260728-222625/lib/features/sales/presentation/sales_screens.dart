...
class _SalesScreenState extends ConsumerState<SalesScreen> {
  ...
  Future<void> _bulkVoid(String businessId) async {
    ...
    if (!mounted) return;
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(content: Text('Voided $count sales.')),
    );
    _clearSelection();
  }
}
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return history.when(
      ...
      error: (_, _) => AppErrorState(
        message: 'Could not load sales history.',
        onRetry: () =>
            ref.invalidate(sales.salesHistoryProvider(businessId)),
      ),
      data: (saleItems) {
                  ...
                  itemBuilder: (context, index) {
                    ...
                    return SelectableDismissibleTile(
                      ...
                      onDismissed: (id) async {
                        ...
                        try {
                          await ref.read(sales.salesRepositoryProvider).voidSale(
                                businessId,
                                id,
                                reason: 'Removed from sales list',
                              );
                      ...
                      child: GestureDetector(
                        onLongPress:
                            canVoid ? () => onEnterSelection(sale.saleId) : null,
                        child: tile,
                      ),
                    )
                        .animate()
                                  ...
                                  children: <Widget>[
                                    ...
                                    if (cartItem != null)
                                      _QuantityStepper(
                                        ...
                                        unit: cartItem.unit,
                                        quantityInput: cartItem.quantityInput,
                                        onIncrease: () => ref
                                            .read(saleCartProvider.notifier)
                                            .addProduct(product),
                                        onDecrease: () => ref
      ...
      builder: (sheetContext) {
        return _AddCustomItemSheet(
          onSubmit: (name, quantityText, unitPriceText) {
            ...
            return ref.read(saleCartProvider.notifier).addCustomItem(
              name: name,
