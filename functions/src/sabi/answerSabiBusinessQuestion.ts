...
import {businessAnswerSystemPrompt} from "./prompts";
import {businessQuestionRequestSchema} from "./schemas";
...
function detectMetric(question: string): {metric: string; period: string} {
  ...
  const period = mentionsWeek
    ...
    : mentionsMonth
      ...
      : mentionsToday
        ...
        : "today";
  const countPeriod =
    mentionsWeek || mentionsMonth || mentionsToday ? period : "all";
  const asksCount =
    q.includes("how many") ||
...
async function loadVerifiedMetric(
  businessId: string,
  metric: string,
  period: string,
  ...
  const currencySymbol =
    (businessSnap.data()?.currencySymbol as string | undefined) ?? "Le";
  ...
  const sales = salesSnap.docs.filter((doc) => {
    const status = (doc.data().status as string | undefined) ?? "completed";
...
export const answerSabiBusinessQuestion = onCall(
  {
    region: "us-central1",
  ...
  async (request) => {
    ...
    const parsed = businessQuestionRequestSchema.safeParse(request.data);
    if (!parsed.success) {
      throw invalidArgument("Please ask a clearer business question.");
    }
    ...
    if (detected.metric === "unknown") {
      return {
        verified: false,
        answer:
          "I couldn’t verify that from your business records. Try asking about today’s sales, low stock, or customer balances.",
        metric: null,
      };
    }
        ...
        const response = await fetch(
          "https://api.groq.com/openai/v1/chat/completions",
          {
            ...
            body: JSON.stringify({
              ...
              temperature: 0.2,
              messages: [
                {role: "system", content: businessAnswerSystemPrompt()},
                {
                  role: "user",
