...
exports.deliverMany = deliverMany;
const firestore_1 = require("firebase-admin/firestore");
const messaging_1 = require("firebase-admin/messaging");
const ids_1 = require("./ids");
const policy_1 = require("./policy");
function safe(value, maximum = 200) {
    return (value ?? "").trim().slice(0, maximum);
}
function payloadData(input, inAppId) {
    const routeParameters = Object.fromEntries(Object.entries(input.routeParameters ?? {})
...
async function tokensForUser(userId) {
    ...
    if (active.length > 0)
        return [...new Set(active)].slice(0, 500);
    const user = await db.collection("users").doc(userId).get();
    const legacy = user.data()?.fcmTokens;
    return [...new Set(Array.isArray(legacy) ? legacy.filter((token) => typeof token === "string" && token.length > 10) : [])].slice(0, 500);
}
async function deliverNotification(input) {
    ...
    const policy = (0, policy_1.resolveNotificationPolicy)(businessPreferences, legacyPreferences, input.category);
    const channels = (0, policy_1.channelDecision)(policy, new Date());
    const deliveryId = (0, ids_1.ledgerId)(input.eventId, input.userId);
    const inAppId = (0, ids_1.notificationId)(input.eventId, input.userId);
...
async function deliverMany(inputs) {
