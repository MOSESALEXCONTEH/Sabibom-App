...
final activeBusinessProvider = StreamProvider<ActiveBusinessState>((ref) {
  ...
  profileAsync.when(
    ...
    error: (error, stackTrace) {
      ...
      controller.add(const ActiveBusinessFailure('Could not load your user profile.'));
      controller.close();
    },
    data: (user) {
      ...
      final subscription = FirebaseFirestore.instance
        ...
        (snapshot) {
          if (!snapshot.exists || snapshot.data() == null) {
            controller.add(const ActiveBusinessFailure('This business profile could not be found.'));
          } else {
            controller.add(ActiveBusinessData(Business.fromFirestore(snapshot.data()!)));
          }
        },
        onError: (error, stackTrace) {
          _log('business', error, stackTrace, businessId);
          controller.add(ActiveBusinessFailure(_friendlyFirestoreError(error)));
        },
      );
...
final dashboardRepositoryProvider = Provider<DashboardRepository>((ref) => FirestoreDashboardRepository());

final dashboardPeriodProvider = NotifierProvider<DashboardPeriodController, DashboardPeriod>(DashboardPeriodController.new);
...
class DashboardRequest {
  const DashboardRequest({required this.businessId, required this.period, required this.currencyCode, required this.currencySymbol});
  ...
  @override
  bool operator ==(Object other) => other is DashboardRequest && other.businessId == businessId && other.period == period && other.currencyCode == currencyCode && other.currencySymbol == currencySymbol;

  @override
  int get hashCode => Object.hash(businessId, period, currencyCode, currencySymbol);
}

final dashboardSummaryProvider = FutureProvider.family<DashboardSummary, DashboardRequest>((ref, request) {
...
final recentActivityProvider = StreamProvider.family<List<DashboardActivity>, String>((ref, businessId) {
  return ref.read(dashboardRepositoryProvider).watchRecentActivity(businessId: businessId);
});

final lowStockProvider = StreamProvider.family<List<ProductStockPreview>, String>((ref, businessId) {
  return ref.read(dashboardRepositoryProvider).watchLowStock(businessId: businessId);
});

final customerBalancesProvider = StreamProvider.family<List<CustomerBalancePreview>, String>((ref, businessId) {
  return ref.read(dashboardRepositoryProvider).watchCustomerBalances(businessId: businessId);
});
...
String _friendlyFirestoreError(Object error) {
  if (error is FirebaseException) {
    return switch (error.code) {
      'permission-denied' => 'We could not access this business. Check your account permissions.',
      'unavailable' => 'Your latest business information is temporarily unavailable.',
...
void _log(String context, Object error, StackTrace stackTrace, String? businessId) {
