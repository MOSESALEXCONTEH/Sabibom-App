...
class _PurchasesScreenState extends ConsumerState<PurchasesScreen> {
    ...
    if (active is! ActiveBusinessData) {
      return Scaffold(
        ...
        body: AppEmptyState(
          ...
          description:
              'Create or select a business before recording purchases.',
          ...
          return ListView.separated(
            ...
            itemBuilder: (context, index) {
              ...
              final tile = AppListRow(
                ...
                leading: const AppListAvatar(icon: Icons.inventory_2_outlined),
                ...
                trailing: Text(
                  ...
                  style: TextStyle(
                    ...
                    decoration: canVoid ? null : TextDecoration.lineThrough,
                  ),
                ),
              );
              return SelectableDismissibleTile(
                ...
                confirmMessage:
                    '“${purchase.purchaseNumber}” will be marked voided. Stock and unpaid supplier balance will be reversed when safe.',
                confirmLabel: 'Void',
                  ...
                  if (branchId == null) {
                    if (mounted) {
                      ScaffoldMessenger.of(context).showSnackBar(
                        const SnackBar(
                          content: Text(branchWriteBlockedMessage),
                        ),
                      );
                    }
                    return false;
  ...
  Future<void> _bulkVoid(String businessId) async {
    ...
    final ok = await confirmListDelete(
      ...
      message:
          'Selected purchases will be marked voided. Stock and unpaid supplier balance will be reversed when safe.',
      confirmLabel: 'Void',
    );
    if (!ok) return;
    ...
    ScaffoldMessenger.of(
      context,
    ).showSnackBar(SnackBar(content: Text('Voided $count purchases.')));
    _clearSelection();
  }
}
...
class _NewPurchaseScreenState extends ConsumerState<NewPurchaseScreen> {
  ...
  Future<void> _submit(dynamic business) async {
    ...
    final branch = branchSelection.branches
          ...
          return ListView(
            ...
            children: <Widget>[
              ...
              if (value.branchCodeSnapshot?.isNotEmpty == true)
                Text('Branch Code: ${value.branchCodeSnapshot}'),
              const SizedBox(height: 16),
              ...value.items.map(
                (item) => ListTile(
